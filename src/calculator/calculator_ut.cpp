...
TEST_F(CalculatorTest, StringConcatenation) {
    EXPECT_EQ(safeGet<std::string>(calc.evaluate("\"Hello\" + \" World\"")), "Hello World");
}
...
TEST_F(CalculatorTest, ComplexExpressionWithParentheses) {
    EXPECT_DOUBLE_EQ(safeGet<double>(calc.evaluate("3.5 + (2 * (1 + 3)) - 4 / 2")), 3.5 + (2 * (1 + 3)) - 4 / 2);
}

TEST_F(CalculatorTest, NestedParentheses) {
    EXPECT_DOUBLE_EQ(safeGet<int>(calc.evaluate("((2 + 3) * (4 - 1)) / 2")), ((2 + 3) * (4 - 1)) / 2);
}

TEST_F(CalculatorTest, MixedIntegerAndDouble) {
    EXPECT_DOUBLE_EQ(safeGet<double>(calc.evaluate("5 + 3.2 * 2 - 1.5 / 0.5")), 5 + 3.2 * 2 - 1.5 / 0.5);
}
...
TEST_F(CalculatorTest, ComplexLogicalAndArithmetic) {
    ...
    EXPECT_EQ(safeGet<bool>(calc.evaluate("((10 / 2) == 4) && (2 > 1)")), false);
}

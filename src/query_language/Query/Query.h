...
class Query {
   public:
    explicit Query(std::string& raw_string) {
        ...
        //     if (std::find(possible_key_words.begin(),
        //     possible_key_words.end(),
        //                   token) != possible_key_words.end()) {
        //         if (!key_words.empty() && !current_param.empty()) {
        //             if (current_param.back() == ' ' ||
        //                 current_param.back() == ';') {
    ...
    QueryType get_query_type() { return m_query_type; }

    std::vector<std::string> get_params() { return raw_params; }

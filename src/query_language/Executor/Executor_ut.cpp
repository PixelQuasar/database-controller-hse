...
TEST_F(ExecutorTest, ExecuteInsertWithExpression1) {
...
TEST_F(ExecutorTest, ExecuteInsertWithBoolean1) {
...
TEST_F(ExecutorTest, ExecuteInsertWithInvalidType1) {
    auto createStmt = "CREATE TABLE Test (ID INT, Name VARCHAR);";
    auto result1 = executor.execute(createStmt);
    EXPECT_TRUE(result1.is_ok());
    auto insertStmt = Parser::parse("INSERT INTO Test VALUES (\"Alice\", 1);");
...
TEST_F(ExecutorTest, ExecuteCreateTableDuplicate1) {
    auto createStmt = "CREATE TABLE Test (ID INT, Name VARCHAR);";
    executor.execute(createStmt);
...
TEST_F(ExecutorTest, ExecuteInsertIntoNonExistentTable1) {
    auto insertStmt = "INSERT INTO NonExistent VALUES (1, \"Alice\");";
    auto result = executor.execute(insertStmt);
    EXPECT_FALSE(result.is_ok());
}

TEST_F(ExecutorTest, ExecuteComplexInsert1) {
    auto createStmt = "CREATE TABLE Test (ID INT, Name VARCHAR, Age INT, Active BOOL);";
    executor.execute(createStmt);

    auto insertStmt = "INSERT INTO Test VALUES (1, \"Alice\", 20 + 5, true && false);";
    executor.execute(insertStmt);
...
TEST_F(ExecutorTest, ComplexScenario1) {
    auto createStmt = 
        "CREATE TABLE Employees ("
        "    ID INT,"
        "    FirstName VARCHAR,"
...
TEST_F(ExecutorTest, UniqueConstraint1) {
...
TEST_F(ExecutorTest, AutoIncrement1) {
...
TEST_F(ExecutorTest, MultipleAutoIncrement1) {
    auto createStmt = "CREATE TABLE Test (ID1 INT AUTOINCREMENT, ID2 INT AUTOINCREMENT, Name VARCHAR);";
    executor.execute(createStmt);
...
TEST_F(ExecutorTest, KeyConstraint1) {
